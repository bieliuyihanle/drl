...
# from gymnasium.wrappers import GymnasiumToGymV0Compatibility
...
def create_env(env_name, config=None, n_workers=1, seed=None, **kwargs):
    """
    Creates a vectorized environment with proper seed handling.
    """
    ...
    def make_rl(rank):
        def _init():
            ...
            env.is_vector_env = (n_workers > 1)
            # 设置种子（如果提供）
            if seed is not None:
                env_seed = seed + rank  # 每个子进程分配不同的种子
    ...
    def make_gym(rank):
        def _init():
            ...
            # env = GymnasiumToGymV0Compatibility(env)
            env.is_vector_env = (n_workers > 1)
            # 设置种子（如果提供）
            if seed is not None:
                env_seed = seed + rank  # 每个子进程分配不同的种子
    ...
    # 从配置中获取参数（如果 config 存在）
    if config is not None:
        n_workers = config.get("main", {}).get("n_workers", 1)
        seed = config.get("main", {}).get("seed", None)

    # 确定环境类型（假设 get_env_type 已定义）
    env_type = get_env_type(env_name)
    if env_type == "gym":
        env_fns = [make_gym(rank=i) for i in range(n_workers)]
    elif env_type == "rl":
        env_fns = [make_rl(rank=i) for i in range(n_workers)]
    else:
        raise ValueError(f"Unsupported environment type: {env_type}")

    # 创建向量化环境
    if n_workers > 1:
        vectorized = SubprocVecEnv(env_fns, start_method="spawn")
    else:
        vectorized = DummyVecEnv(env_fns)

    # 可选：添加标准化和帧堆叠
    if config is not None:
        if config.get("main", {}).get("normalize", False):
            vectorized = VecNormalize(vectorized, clip_obs=1, clip_reward=1)
        if config.get("main", {}).get("frame_stack", 0) > 0:
